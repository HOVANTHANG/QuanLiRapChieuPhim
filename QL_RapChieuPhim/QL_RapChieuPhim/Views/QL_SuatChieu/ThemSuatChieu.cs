...
namespace QL_RapChieuPhim.Views
{
    public partial class ThemSuatChieu : Form
    {
        ...
        //form đc kéo bằng chuột
        [DllImport("user32.DLL", EntryPoint = "ReleaseCapture")]
        private extern static void ReleaseCapture();
        [DllImport("user32.DLL", EntryPoint = "SendMessage")]
        private extern static void SendMessage(System.IntPtr hwnd, int wmsg, int wparam, int lparam);
        
        ChoPhepKyTu cpkt = new ChoPhepKyTu(); //giới hạn ký tự nhập vào textbox
        ...
        // kéo form khi giữ chuột trái ở bất kỳ vị trí nào trên form.
        ...
        //hàm kiểm tra trùng lặp bằng thủ tục
        ...
        private void btn_luuPhim_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrorMes.SetError(cboTenPhim, "Hãy nhập t!");
            }
            else if (string.IsNullOrWhiteSpace(cboPhongChieu.Text))
            {
                ErrorMes.Clear();
                ErrorMes.SetError(cboPhongChieu, "Hãy nhập phòng!");

            }
            else if (string.IsNullOrWhiteSpace(txtGiaVe.Text))
            {
                ErrorMes.Clear();
                ErrorMes.SetError(txtGiaVe, "Hãy nhập giá vé!");
            }
			else if (!decimal.TryParse(txtGiaVe.Text, out _))
			{
				ErrorMes.SetError(txtGiaVe, "Giá vé phải là số!");
				return;
			}
			else
            {
                ...
                else
                {
                    ...
                    //cập nhật tg kết thúc nếu có dữ liệu phim
                    if (result.Rows.Count > 0)
                    {
                        if (int.TryParse(chiDuLai, out int thoiLuongPhim))
                        {
                            Console.WriteLine(thoiLuongPhim);
                            //cộng thời lượng phim vào giờ bắt đầu để ra giờ kết thúc
                   ...
                   // string maVe = ft.SinhMaTuDong("tbChiTietVe", 'V', "MaVe");
					...
					//
					qlsc.dgv_SuatChieu.DataSource = dataBase.DataRead("SELECT " +"XC.MaXuatChieu as [Mã suất chiếu], P.TenPhim AS [Tên Phim], " +"TL.LoaiPhim AS [Loại Phim], P.ThoiLuongPhim AS [Thời Lượng], " +"XC.CaChieu AS [Giờ Chiếu], XC.NgayChieu AS [Ngày Chiếu] " +"FROM tbPhim AS P " +"INNER JOIN tbTheLoaiPhim AS TL ON P.MaTheLoai = TL.MaTheLoai " +"INNER JOIN tbXuatChieu AS XC ON P.MaPhim = XC.MaPhim " +"WHERE XC.NgayChieu = '" + dtNgayChieu + "'");

					 
					// Đảm bảo cột chỉnh sửa nằm ở vị trí cuối cùng
					if (qlsc.dgv_SuatChieu.Columns.Contains("EditColumn"))
					{
						// Di chuyển cột chỉnh sửa về cuối
						qlsc.dgv_SuatChieu.Columns["EditColumn"].DisplayIndex = qlsc.dgv_SuatChieu.Columns.Count - 1;
					}
				 

					// Cập nhật trạng thái nút lưu phim
					if (!string.IsNullOrWhiteSpace(lblTenPhim.Text))
						btn_luuPhim.Enabled = false;
        ...
        private void panel_ChuThich_ControlAdded(object sender, ControlEventArgs e)
        {
