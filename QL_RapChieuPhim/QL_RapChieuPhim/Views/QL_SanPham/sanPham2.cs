    ...
    public partial class sanPham2 : Form
    {
        ...
        private void sanPham2_Load(object sender, EventArgs e)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                button1.Click += Button3_Click; //Button3_Click được gọi để xử lý sự kiện nhấp chuột này
        ...
        // Hàm thêm một panel mới vào FlowLayoutPanel mà ko cần load lại toàn bộ dữ liệu
        ...
        public void RemovePanelByMaSP(string maSP)
        {
            ...
            {
                // Kiểm tra xem control có phải là panel không
                if (control is Bunifu.UI.WinForms.BunifuPanel panel)
                {
                    // Lấy MaSP từ panel (đã lưu trong Tag của panel ở load)
        ...
        public void UpdatePanelByMaSP(string maSP, string tenSanPham, string soLuong, string donGia, string anh)
        {
            foreach (Control control in flowLayoutPanel1.Controls)
            {
                ...
                if (control is Bunifu.UI.WinForms.BunifuPanel panel)
                {
                    // Lấy MaSP từ panel (đã lưu trong Tag của panel ở đầu)
                    string panelMaSP = control.Tag as string;

                    // So sánh MaSP của panel với MaSP mà bạn muốn cập nhật
                    if (panelMaSP == maSP)
                    {
                        // Tìm và cập nhật các thành phần trong panel
                        foreach (Control panelControl in panel.Controls)
                        {
                            //cập nhật label dựa vào vị trí
        ...
        // Hàm xử lý sự kiện khi nhấn nút Edit mà ko phải tải lại toàn bộ trang (sau khi load trang)
		...
		private void Button4_Click(object sender, EventArgs e)
		{
			if (sender is Control control && control.Parent is Bunifu.UI.WinForms.BunifuPanel panel)
			{
				...
				if (MessageBox.Show("Bạn có chắc chắn xóa mã sản phẩm này không?", "Xóa sản phẩm", MessageBoxButtons.YesNo) == DialogResult.Yes)
				{
					string imagePath = Application.StartupPath + "\\img\\" + maSP + ".jpg"; // Hoặc định dạng phù hợp
					if (System.IO.File.Exists(imagePath))
					{
        ...
        {
            string keyword = txt_timkiemSP.Text.Trim().ToLower();// Lấy từ khóa tìm kiếm

            foreach (Control control in flowLayoutPanel1.Controls) // Duyệt qua tất cả các control trong FlowLayoutPanel
            {
                if (control is Bunifu.UI.WinForms.BunifuPanel panel)
                {
